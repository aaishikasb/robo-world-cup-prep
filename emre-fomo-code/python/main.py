...
from camera import _start_camera, _copy_frame, _detect_wall, get_preview_image
...
if _index.exists():
    ui = WebUI(assets_dir_path=_index.parent)
    ui.expose_api("GET", "/preview", get_preview_image)
    print("[INFO] web UI started")
...
def loop() -> None:
    ...
    if frame is not None:
        wall_side, coverage = _detect_wall(frame)
        print(f"wall       : {wall_side}  coverage={coverage}")
